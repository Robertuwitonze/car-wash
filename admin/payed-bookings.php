...
?>
    ...
    if ($_GET['bookingid']) {
        ...
        $sql = "delete from tblcarwashbooking where bookingId=:id";
        $query = $dbh->prepare($sql);
        $query->bindParam(':id', $id, PDO::PARAM_STR);
        $query->execute();

        echo "<script>alert('Deleted successfully');</script>";
        echo "<script>window.location.href ='payed-bookings.php'</script>";
    }
    ?>
    <!DOCTYPE HTML>
    <html>
                            ...
                            <table id="example1" class="table table-bordered table-striped">
                                <thead>
                                    <tr>
                                        ...
                                        <th style="color:black;">Posting date </th>
                                ...
                                <tbody>
                                    <?php $sql = "SELECT *,tblcarwashbooking.id as bid, tblcarwashbooking.paymentMode as mode 
                                    from tblcarwashbooking join tblwashingpoints on tblwashingpoints.id=tblcarwashbooking.carWashPoint
                                     where status='Completed' and paymentStatus='payed' and  tblcarwashbooking.carWashPoint='$_SESSION[carwash]'";
                                    $query = $dbh->prepare($sql);
                                    $query->execute();
                                    ...
                                    if ($query->rowCount() > 0) {
                                        foreach ($results as $result) {                ?>
                                            <tr>
                                                ...
                                                <td style="color:black;"><?php echo htmlentities($result->mode); ?></td>

                                                <td style="color:black;"><a href="?bid=<?php echo htmlentities($result->bid); ?>&&bookingid=<?php echo htmlentities($result->bookingId); ?>"><input type="button" class="btn btn-danger" value="Delete"></a>
                                                </td>
                                            <?php } ?>
                                            </tr>
                                        <?php } else { ?>
