...
if (strlen($_SESSION['alogin']) == 0) {
    ...
    if (isset($_POST['submit'])) {
        ...
        if ($password == $passwordconf) {
            $sql = "update  tblusers set names=:names,username=:username,password=:password,email=:email,carwash=:carwash where id=:id";
            ...
            <div class="left-content">
                ...
                <div class="grid-form">
                    ...
                    <div class="grid-form1">
                        <h3>Edit Washing Point Owner</h3>
                        <?php
                        $id = $_GET['wpid'];
                                ...
                                <div class="tab-pane active" id="horizontal-form">
                                    <form class="form-horizontal" name="washingpoint" method="post" enctype="multipart/form-data">
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-8">
                                                ...
                                                <select class="form-control" name="washingpointname" id="washingpointname" required>
                                                    ...
                                                    $psql = "SELECT * from tblwashingpoints where id='$result->carwash' and is_active=1";
                                                    $pquery = $dbh->prepare($psql);
                                                    $pquery->execute();
                                                    $presults = $pquery->fetchAll(PDO::FETCH_OBJ);

                                                    foreach ($presults as $presult) {                ?>
                                                        <option value="<?= $result->carwash ?>"><?= $presult->washingPointName; ?></option>
                                                    <?php } ?>

                                                    <?php
                                                    $wpsql = "SELECT * from tblwashingpoints where is_active=1 and id not in(SELECT carwash from tblusers where is_active=1)";
                                                    $wpquery = $dbh->prepare($wpsql);
                                                    $wpquery->execute();
                                                    $wpresults = $wpquery->fetchAll(PDO::FETCH_OBJ);
                                                    foreach ($wpresults as $wpresult) {
                                                    ?>
                                                        <option value="<?= $wpresult->id ?>"><?= $wpresult->washingPointName ?></option>
                                                    <?php } ?>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-8">
                                                <input type="text" class="form-control" name="names" id="names" value="<?= $result->names; ?>" required>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-8">
                                                <input type="password" class="form-control" name="password" id="password" value="<?= $result->password; ?>" required>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label for="focusedinput" class="col-sm-2 control-label">Confirm Password</label>
                                            <div class="col-sm-8">
                                                <input type="password" class="form-control" name="passwordconf" id="passwordconf" value="" required>
                                            </div>
                                        </div>
